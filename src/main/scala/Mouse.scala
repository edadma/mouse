...
object Mouse {
	...
	private class Env( init: Array[Any], macros: Map[Char, Int] ) {
		val stack = new Stack
	...
	private class Call( m: Char ) extends Instruction( "call '" + m + "'" ) {
		def execute( env: Env ) {
			
		}
	}
	
	private object Execute extends Instruction( "execute" ) {
		def execute( env: Env ) {
	...
	def run( program: (IndexedSeq[Instruction], Map[Char, Int]), init: Any* ) = {
	val env = new Env( init.toArray, program._2 )		
		
		while (env.pc >= 0 && env.pc < program._1.length) {
		val inst = program._1(env.pc)
		
			inst.execute( env )
			
			if (env.trace) {
				println( (env.pc, inst, env.stack.toIndexedSeq, env.vars map {case (k, v) => k -> v.v} toMap) )
			}
			
			env.pc += 1
		}
	
		(env.stack.toIndexedSeq, env.vars map {case (k, v) => k -> v.v} toMap)
	}
	...
	val macros = new HashMap[Char, Int]
		...
		inst
		(program toIndexedSeq, macros toMap)
	}
}